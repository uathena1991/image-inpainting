...
cdef double[:,:] get_patch(int cntr_ptx,
                           int cntr_pty,
                           double[:,:] img,
                           int patch_size):
    """
    ...
    cdef:
        ...
        int p = patch_size / 2
        int dim = 2*p+1
        np.ndarray[DTYPE_t, ndim=2] np_patch = np.zeros((dim, dim))
        double[:,:] patch = np_patch
        int i

    for i in prange(p+1, nogil=True, num_threads=4):
        patch[x-i,y-i] = img[x-i,y-i]
        patch[x+i,y+i] = img[x+i,y+i]
